...
abstract class AbstractCommandLineRProcess extends AbstractRProcess {
    ...
    function doStart() {
        ...
        $this->process = proc_open(
                sprintf("\"%s\" --silent --vanilla", $this->rCommand), $descriptors, $this->pipes
        );
    ...
    function doWrite(array $rInputLines) {
        ...
        foreach ($rInputLines as $rInputLine) {
            ...
            $currentCommandInput .= fread(
                    $this->pipes[1], $this->infiniteLength
            );
            ...
            if ($currentCommandErrorOutput) {
                $error = new RError(
                        $this->inputLineCount - 1, count($this->inputLog) - 1, $currentCommandInput, $currentCommandErrorOutput
                );
